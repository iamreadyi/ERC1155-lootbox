...
contract SimpleLoot {
    ...
    function lootTheBox(uint256 boxId, uint256 testSeed) public {
        ...
        /*  if (0 <= server_seed && server_seed < typeOdds[0]) {
            uint256 initial = 0;
            for (uint256 i = 0; i < typeCounts[0]; i++) {
                if (initial <= server_seed && server_seed < initial + odds[i]) {
                    //require(gamiBox.balanceOf(tokensSeller, boxItems[boxId][i]) >= 1);
                    gamiBox.safeTransferFrom(
                        tokensSeller,
                        msg.sender,
        ...
        } else if (typeOdds[0] <= server_seed && server_seed < typeOdds[1]) {
            uint256 initial = typeOdds[0];
            for (uint256 i = 0; i < typeCounts[1]; i++) {
                if (
                    initial <= server_seed &&
                    server_seed < initial + odds[typeCounts[0] + i]
                ) {
                    gamiBox.safeTransferFrom(
                        tokensSeller,
                        msg.sender,
                        boxItems[boxId][typeCounts[0] + i],
                        1,
                        ""
                    );
                    break;
                } else {
                    initial += odds[typeCounts[0] + i];
                }
            }
        } else if (typeOdds[1] <= server_seed && server_seed < typeOdds[2]) {
            uint256 initial = typeOdds[1];
            for (uint256 i = 0; i < typeCounts[2]; i++) {
                if (
                    initial <= server_seed &&
                    server_seed <
                    initial + odds[typeCounts[0] + typeCounts[1] + i]
                ) {
                    gamiBox.safeTransferFrom(
                        tokensSeller,
                        msg.sender,
                        boxItems[boxId][typeCounts[0] + typeCounts[1] + i],
                        1,
                        ""
                    );
                    break;
        ...
        } else if (typeOdds[2] <= server_seed && server_seed < typeOdds[3]) {
            uint256 initial = typeOdds[2];
            for (uint256 i = 0; i < typeCounts[3]; i++) {
                if (
                    initial <= server_seed &&
                    server_seed <
                    initial +
                        odds[typeCounts[0] + typeCounts[1] + typeCounts[2] + i]
                ) {
                    gamiBox.safeTransferFrom(
                        tokensSeller,
                        msg.sender,
                        boxItems[boxId][
                            typeCounts[0] + typeCounts[1] + typeCounts[2] + i
                        ],
                        1,
                        ""
                    );
        ...
        } else if (typeOdds[3] <= server_seed && server_seed < typeOdds[4]) {
            uint256 initial = typeOdds[3];
            for (uint256 i = 0; i < typeCounts[4]; i++) {
                if (
                    initial <= server_seed &&
                    server_seed <
                    initial +
        ...
        } else if (typeOdds[4] <= server_seed && server_seed < typeOdds[5]) {
            uint256 initial = typeOdds[4];
            for (uint256 i = 0; i < typeCounts[5]; i++) {
                if (
                    initial <= server_seed &&
                    server_seed <
                    initial +
